...
### 4.4 手动推送至 FastGPT 知识库

**权限**：管理员

> 将案例全文与元数据推送到 FastGPT 知识库（`chunk` 模式），用于 RAG。要求 `ai_status = 2`；与翻译/摘要等任务一样为**异步**，提交后可通过案例详情中的 `fastgptSyncStatus` 等字段查看结果。

```
POST /cases/{id}/sync-fastgpt
```
...
{
  "code": 200,
  "message": "知识库同步任务已提交，正在后台处理"
}
```

> 已逻辑删除的案例不可调用；若同步状态为「同步中」会返回 3006。重复调用会在知识库中新增多个集合，请按需使用。
